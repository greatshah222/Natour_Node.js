...
var displayMap = function displayMap(locations) {
  ...
  mapboxgl.accessToken = 'pk.eyJ1IjoiYmlzaGx1IiwiYSI6ImNrYTZudXY4eDAxcWYyeG16dHBrcmFqNXgifQ.sP57BolySBydnCi36njfQg';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/bishlu/cka6oeop70r521iphjc2uam6l',
    scrollZoom: false //   center: [25.4651, 65.0121],
    //   zoom: 6,
  ...
  var bounds = new mapboxgl.LngLatBounds(); // loc has cordinates and by defauly in mapbox it needs to be an array
  // we are looping through all the location so that we can add it to the map

  locations.forEach(function (loc) {
    // create  marker to that location
    var el = document.createElement('div');
    el.className = 'marker'; // it is specified in the css
    ...
    */
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ":").concat(loc.description, " </p>")).addTo(map); // they overlap the marker so we need to set an offset property into the Popup;
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); // where we want to use this in the body afterbegin

  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // also hide alert after 3 s
...
var stripe = Stripe('pk_test_J8nZHumuTjy4Z1lP54yi1XCD00ZiUSkPG3');
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            console.log(tourId); // 1) get the chekout session from the server
          ...
          case 4:
            ...
            console.log(session); // 2) create the checkout form and charge the credit card for us
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 9:
            ...
            (0, _alert.showAlert)('error', _context.t0.response.data.message); // will give our appErrror error
...
var loginForm = document.querySelector('.form--login'); // for logout

var logOutBtn = document.querySelector('.nav__el--logout'); // for updating data

var updateUserForm = document.querySelector('.form-user-data'); // for password change
...
if (loginForm) {
  loginForm.addEventListener('submit', function (e) {
    e.preventDefault(); // the value of this email and pwd can only be accessed in event listner
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              passwordConfirm = document.getElementById('password-confirm').value; // await cause update Settings is async function and to clear we need await
